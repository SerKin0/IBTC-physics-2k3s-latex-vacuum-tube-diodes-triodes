...
_y = [3.78,10.16,16.88,24.84,32.40,40.90,49.94,62.40,73.52,86.31]
_x = [u for u in range(2, 21, 2)]

# Данные с единицами измерения на русском
y = Values(
    _y,
    [0.002*i + 2*0.01 for i in _y],
    name="I_a",
    units=r"\text{мА}",
    step=10,
    roundoff=2,
...
# y = Values(
#     [8.6, 14.3, 20.0, 25.7, 37.1, 48.6, 60.0, 71.4, 85.7, 100.0, 111.4, 122.9, 134.3, 145.7, 151.4, 157.1],
#     [0.014693877551020411,0.024489795918367353,0.034285714285714294,0.04408163265306123,0.06367346938775512,0.08326530612244899,0.10285714285714287,0.12244897959183676,0.1469387755102041,0.17142857142857146,0.19102040816326535,0.2106122448979592,0.2302040816326531,0.249795918367347,0.25959183673469394,0.2693877551020409],
#     name=r"\varphi",
#     units=r"\text{рад}",
#     step=10,
#     flag_error_label=False)
...
#     [0.4123105625617661, 0.4123105625617661, 0.4123105625617661,0.4123105625617661, 0.4123105625617661, 0.1, 0.4123105625617661,0.1, 0.1, 0.4123105625617661, 0.1, 0.4123105625617661,0.9055385138137417, 0.4123105625617661, 1.3038404810405297, 2.6019223662515376],
#     name="t",
#     step=1,
#     units=r"\text{с}",
#     flag_error_label=False)

with PdfPages('graph_oberbek_pendulum_2.pdf') as pdf:
    ...
    # Теоретическая линия
    x_theory = np.linspace(min(x.data), max(x.data))
